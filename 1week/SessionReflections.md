...
## 6. JVM의 역할
```
만약에 JVM이 없다면 우리가 소스를 컴파일 했을 때 나올 결과는 '기계어'일 것이다. 이 기계어는 해당 OS가 이해할 수 있는 언어로 번역 된 것일텐데
이렇게 된다면 다른 OS에서 컴파일된 바이트 코드를 사용할 수 없게 된다. 컴퓨팅 환경이 바뀔때 마다 '매번 컴파일'을 해줘야 하는 불편함이 생긴다.
```

```
그래서 자바에서는 JVM이라는 가상의 컴퓨터, 즉 프로그램을 중간에 두어 '한번 컴파일 된 코드를' 운영체제에 독립적으로 실행시킬 수 있도록 지원한다.
또 js, kotlin, java등 .. 언어에 국한되지 않고 컴파일된 결과가 JVM이 이해할 수 있는 코드인 JavaByteCode의 형태로만 나온다면 언어에도 독립적으로
개발할 수 있게되는 이점이 있다.
```
...
[Interpreter / JIT Compiler의 역할]
즉 Interpreter는 JavaByteCode를 CPU가 이해할 수 있 기계어로 '실시간 통역'을 해주는 녀석이고 JIT는 실시간 통역에서 많이 쓰이는 구문들을 미리 '번역'해둬서
통역사(Interpreter)가 굳이 기억하지 않아도 통역 대상(컴퓨터)에게 쉽게 전달할 수 있도록 돕는 역할이다.
```
