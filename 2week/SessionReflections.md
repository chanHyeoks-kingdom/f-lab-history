...
> [@Serial GC]
>
> - 1. 과거 CPU가 한개일 때 만들어진 GC이다. 그래서 현대의 멀티 코어나 멀티 CPU 시스템에는 최적화된 성능을 보장할 수 없다.
> - 2. 때문에 지금은 잘 쓰지 않는 방식이지만, Device 성능이 제한적이어서 하나의 코어만 사용하는 시스템에는 적용을 고려해볼 수 있을 거 같다.
> - 3. STW(Stop-The-World) 시간도 다른 GC에 비해 비교적 긴 편이다.
