...
1. 서비스에서 DTO 변환토록 한 것이랑, 통합 테스트 개선 PR
2. 공통 모듈 github이용해서 jitpack 시도
3. 기술 질문 최대한 봤고
4. 결제 도메인 구현은 아직 어떻게 접근해야 할지 모르는 상태고
5. 가장 기초적인 DP 문제 생각
6. 문제 상황을 보고 APM 도구 사용하는 거 실습 잠깐 해봤습니다.
...
> b. HTTP를 돌리기 위한 TCP 연결 과정
```
...
```

> c. HTTPS를 돌리기 위한 핸드 셰이크 과정
```
...
```
...
```
- 블록 단위 동기화와 똑같은 구조지만 `전용 락 객체`를 사용한다는 점에서 차이가 있다.
...
> a. 이벤트를 기록한다는 개념이 아직 잘 이해가 안됨. 차주 학습 예정. CQRS는 그냥 읽기, 쓰기를 나누고 DB나누는 거 라는 정도만 이해
-
```
