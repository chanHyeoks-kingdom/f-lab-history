...
### 4. Spring AOP(Aspect-Oriented Programming)에 대해 설명하시오
> Spring AOP는 많은 코드에 공통적으로 들어가는 같은 형태의 코드를 분리하기 위해 고안된 AOP라는 프로그래밍 패러다임을 AspectJ와 SpringFramework의 SpringContext등을 통해 스프링에서 구현한 AOP가 스프링 AOP입니다. 내부적으로 프록시를 이용하고 이를 구현하기 위해 reflect의 java 동적 프록시나 스프링 프레임워크에서 제공하는 CGLIB을 사용합니다.
```
...
##### 꼬리질문 6. 실제 프로젝트에서 AOP를 적용한 예제를 들어줄 수 있나요?
```
