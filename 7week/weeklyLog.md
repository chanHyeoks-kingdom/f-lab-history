...
- 2024-02-22, 일단 뒤에 AOP 설명시에 프록시로 대체
```
...
### 4. Spring AOP(Aspect-Oriented Programming)에 대해 설명하시오
> Spring AOP는 많은 코드에 `공통적으로 들어가는 같은 형태의 코드를 분리`하기 위해 고안된 AOP라는 프로그래밍 패러다임을 `AspectJ와 SpringContext등을 통해 구현한 AOP가 스프링 AOP`입니다. `내부적으로 다이나믹 프록시`를 이용하고 이를 구현하기 위해 자바 표준 java.lang.relfect에서 제공하는 `java 동적 프록시`나 스프링 프레임워크의 내부에서 사용하는 `CGLIB`을 사용합니다.
```
