...
>> 균형잡힌 B-Tree 기반으로 구축되어 있어서 탐색에 기본 O(logN)시간이 걸립니다.
> ###### a. 인덱스는 그냥 빠르게 찾고싶은 필드를 삽입 할 때마다 정렬해서 항상 정렬된 상태를 유지하는 것 입니다. 이렇게 정렬된 상태를 유지하면 균형잡힌 B트리를 통해 빠른 조회를 할 수 있는데요 Clustered index랑 non-clustered index라는 건 이런 인덱스라는 개념을 분류하는 방법 중에 하나입니다.
