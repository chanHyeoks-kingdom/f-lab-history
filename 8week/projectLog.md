...
### 7. 인증 방식
- 인메모리 세션 ID 이용 -> 단일 인스턴스에선 문제가 없었지만 다중 인스턴스를 사용하게되자 여러 문제들이 발생했다.

#### 1. 세션 불일치 문제
> 사용자의 요청이 매번 다른 서버로 라우팅시 각 서버는 별도로 세션을 관리하기 때문에 세션 불일치 문제가 발생할 수 있다. 또 반영을 위해 서버를 재기동했을 때 자연스럽게 세션 정보가 사라지게되는 현상이 생겼다.

```
로드밸런스 레벨에서 세션 스티키니즈를 구현할 수 있지만 서버 장애시 사용자의 세션키 유실로 이어질 수 있다. 그래서 별도 DB를 두고 관리하려 했는데 이는 굉장히 많은 조회 요청이 있을 것이기 때문에
비용면에서 문제가 있을 수 있다. 그래서 캐싱이나 레디스 같은 방법들을 도입해보기로 결정했다.
```
